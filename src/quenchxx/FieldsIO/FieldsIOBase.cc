...
FieldsIOFactory::FieldsIOFactory(const std::string & ioFormat) {
  if (getMakers().find(ioFormat) != getMakers().end()) {
    oops::Log::error() << ioFormat << " already registered in quenchxx::FieldsIOFactory."
      << std::endl;
    throw eckit::Exception("Element already registered in quenchxx::FieldsIOFactory.",
      Here());
  }
...
std::unique_ptr<FieldsIOBase> FieldsIOFactory::create(const std::string & ioFormat) {
  ...
  if (jsb == getMakers().end()) {
    oops::Log::error() << ioFormat << " does not exist in quenchxx::FieldsIOFactory." << std::endl;
    throw eckit::UserError("Element does not exist in quenchxx::FieldsIOFactory.", Here());
  }
